...
class ToMe16_mlp_hd64(nn.Module):
    def __init__(self, config, vision_cfg):
        ...
        self.tvg_mlp = nn.Sequential(nn.Linear(config.mm_hidden_size, config.hidden_size), 
                                     nn.GELU(), 
                                     nn.Linear(config.hidden_size, config.hidden_size))
    ...
    def forward(self, x, compress=False, local_num_frames=-1, return_video_feature=False, video_feature=False, tvg=False): # 单帧64
